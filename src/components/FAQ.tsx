...
type FaqItem = {
	q: string;
	a: string;
	category: "booking_payment" | "rooms_amenities" | "onsite_services" | "house_rules";
};
...
const renderAnswer = (answer: string) => {
	...
	const urlDomainOnly =
		/(?:www\.)?[a-z0-9](?:[a-z0-9-]*[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9-]*[a-z0-9])?)+(?:\/[\w\-._~:/?#[\]@!$&'()*+,;=%]*)?/i;
	...
	return lines.map((line, idx) => <div key={idx}>{linkify(line)}</div>);
};
				...
				<div className="mb-8">
					...
					>
						{t(
							"faq.searchLabel",
							langKey === "en"
								? { defaultValue: "Search FAQ" }
								: { defaultValue: "Пошук по FAQ" },
						)}
					</label>
					<div className="relative">
						<input
							...
							placeholder={t(
								...
								langKey === "en"
									? { defaultValue: "Type to filter questions..." }
									: { defaultValue: "Введіть запит для фільтру..." },
							)}
						/>
						<svg
									...
									return (
										...
										>
											...
											<div
												...
												aria-hidden={!open}
											>
												<div className="px-6 pb-5 pt-0 text-base text-gray-600 leading-relaxed">
													{renderAnswer(item.a)}
												</div>
											</div>
										</div>
				...
				{filtered.length === 0 && (
					<p className="mt-4 text-center text-gray-500">
						{t(
							"faq.noResults",
							langKey === "en"
								? { defaultValue: "No results. Try another query." }
								: { defaultValue: "Нічого не знайдено. Спробуйте змінити запит." },
						)}
					</p>
				)}
				...
				<div className="mt-8 text-center text-sm text-gray-500">
					{t(
						...
						langKey === "en"
							? {
									defaultValue:
										"Didn’t find an answer? Contact us — we’re happy to help.",
								}
							: {
									defaultValue:
										"Не знайшли відповіді? Напишіть нам — із радістю допоможемо.",
								},
					)}
				</div>
			</div>
